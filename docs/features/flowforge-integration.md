...
- [ ] Pin a FlowForge commit for integration tests
...
| Integration | FlowForge → HumanGateway → FlowForge | Run a real FlowForge workflow with `human-input` and `human-approval`; assert resume |
| Comparison | Console vs HumanGateway provider | Same workflow headlessly; assert identical workflow outcome |

Key test scenarios:
1. A FlowForge `human-input` node round-trips through HumanGateway; workflow resumes with the response.
2. A `human-approval` node round-trips with approve/reject + reason.
...
1. A FlowForge `human-input` and `human-approval` node are delivered through HumanGateway and the workflow resumes with the human response and any artifacts (Phase 7 exit).
...
|---|----------|--------------------|
| 1 | Does FlowForge need upstream changes? | Prefer consumer-only adapter; upstream only if unavoidable (product vision Open Q #12) |
