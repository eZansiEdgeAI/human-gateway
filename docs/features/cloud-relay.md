...
| RELAY-FR-01 | FR-28 | Relay stores messages/tasks in PostgreSQL and artifacts in object storage |
...
**ID Prefix:** RELAY
**Summary:** The cloud side: an ASP.NET Core service backed by PostgreSQL and object storage that acts as a relay and rendezvous point. It accepts outbound sync from Edge Gateways (no inbound connectivity needed at the school) and enables cross-school message exchange and remote web access.
...
|----|-------------|----------|
| RELAY-FR-01 | Relay stores messages/tasks in PostgreSQL and artifacts in object storage | Must |
...
### Phase 4: Cloud Relay
- [ ] Scaffold `src/HumanGateway.Relay` (ASP.NET Core minimal API + EF Core/PostgreSQL)
- [ ] PostgreSQL schema: gateways, conversations, messages, deliveries, artifacts, cursors
...
- [ ] Object-storage provider interface (S3-compatible / Azure Blob) for artifacts
- [ ] Docker Compose environment: Relay + PostgreSQL (+ MinIO for dev) + Edge
...
| Integration | Edge ↔ Relay over HTTP | Testcontainers: PostgreSQL + MinIO; assert sync round-trip |
...
| 2 | Object storage in dev? | MinIO container (S3-compatible) |
